{{-- resources/views/pages/admin/laporan/pengguna/cetak.blade.php --}}
<!DOCTYPE html>
<html lang="id">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Cetak Laporan Pelanggan - BPABS</title>
    <style>
        * {
            margin: 0;
            padding: 0;
        ...
        /* REMOVED: Blue marker effect that created the red lines */
        ...
        /* REMOVED: Blue marker signature effect that created decorative lines */
...
<body>
    ...
    <div class="print-header no-print">
        <div>
            <h3 style="margin: 0; color: #374151;">Preview Cetak - Laporan Pelanggan</h3>
            <p style="margin: 5px 0 0 0; color: #6b7280; font-size: 13px;">
                Total {{ $users->count() }} data pelanggan | 
        ...
        <div class="print-controls">
            ...
            <a href="{{ route('admin.laporan.pengguna') }}" class="btn btn-secondary">
    ...
    <div class="content">
        ...
        <div class="table-container">
                    ...
                    <tbody>
                        @foreach($users as $index => $user)
                            <tr>
                                ...
                                <td>
                                    <div style="font-weight: 600;">{{ $user->name ?? $user->nama ?? 'N/A' }}</div>
                                ...
                                <td>
                                    @if($user->telepon ?? $user->no_hp)
                                        <div>📞 {{ $user->telepon ?? $user->no_hp }}</div>
                                    @endif
                                    @if($user->alamat)
                                        <div style="color: #6b7280; font-size: 9px;">{{ Str::limit($user->alamat, 40) }}</div>
                                    @endif
                                </td>
                                <td>
                                    @if($user->trashed())
                                ...
                                <td>
                                    @if($user->meteran && $user->meteran->isNotEmpty())
                                        <div class="meteran-info">
                                            <div class="number">{{ $user->meteran->first()->nomor_meteran }}</div>
                                            <div class="status">Status: {{ ucfirst($user->meteran->first()->status ?? 'N/A') }}</div>
                                        </div>
                                    @else
                                        <div class="no-meteran">Belum ada meteran</div>
                                    @endif
                                </td>
                                <td>
                                    <div>{{ $user->created_at->format('d/m/Y') }}</div>
                                    <div style="font-size: 10px; color: #6b7280;">{{ $user->created_at->format('H:i') }}</div>
                                </td>
                                <td>Pelanggan</td>
                            </tr>
                        @endforeach
            ...
            @else
                <div class="no-data">
                    <h3 style="margin-bottom: 8px; color: #374151;">Tidak ada data pelanggan</h3>
        ...
        <div class="footer">
            ...
            <p>Halaman {{ $users->currentPage() }} dari {{ $users->lastPage() }} | Total {{ $users->total() }} pelanggan</p>
        </div>
    </div>
        ...
        document.addEventListener('keydown', function(e) {
            ...
            if (e.key === 'Escape') {
                if (confirm('Kembali ke halaman laporan?')) {
                    window.location.href = '{{ route("admin.laporan.pengguna") }}';
                }
            }
        });
    </script>
