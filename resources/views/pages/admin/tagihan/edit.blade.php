            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="tarif_per_m3" id="tarif_per_m3" 
                        ...
                        @foreach($tarif as $item)
                            <option value="{{ $item->harga }}" {{ old('tarif_per_m3', $tagihan->tarif_per_kubik) == $item->harga ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <input type="text" id="pemakaian" 
                           value="{{ number_format($tagihan->jumlah_pemakaian, 2) }} m³"
                ...
                <div>
                    ...
                    <select name="status" id="status" 
                        ...
                        <option value="{{ \App\Models\Tagihan::STATUS_BELUM_BAYAR }}" {{ old('status', $tagihan->status) == \App\Models\Tagihan::STATUS_BELUM_BAYAR ? 'selected' : '' }}>Belum Bayar</option>
                        <option value="{{ \App\Models\Tagihan::STATUS_SUDAH_BAYAR }}" {{ old('status', $tagihan->status) == \App\Models\Tagihan::STATUS_SUDAH_BAYAR ? 'selected' : '' }}>Sudah Bayar</option>
                        <option value="{{ \App\Models\Tagihan::STATUS_TERLAMBAT }}" {{ old('status', $tagihan->status) == \App\Models\Tagihan::STATUS_TERLAMBAT ? 'selected' : '' }}>Terlambat</option>
                    </select>
                    @error('status')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
            ...
            <div class="bg-gray-50 p-4 rounded-lg border border-gray-200">
                <div class="flex items-center justify-between">
                    ...
                    <div class="text-right">
                        ...
                        <div class="text-xs text-gray-500" id="detailKalkulasi">
                            {{ number_format($tagihan->jumlah_pemakaian, 2) }} m³ × Rp {{ number_format($tagihan->tarif_per_kubik, 0, ',', '.') }} 
                            = Rp {{ number_format($tagihan->biaya_pemakaian, 0, ',', '.') }}
                            <br>+ Biaya Admin: Rp 5.000
                            <br>Total: Rp {{ number_format($tagihan->total_tagihan, 0, ',', '.') }}
                        </div>
                    </div>
                </div>
            </div>
...
<script>
    ...
    function calculateTagihan() {
        ...
        const biayaAdmin = 5000; // Biaya administrasi tetap
        const totalTagihan = biayaPemakaian + biayaAdmin;
        ...
        if (pemakaian >= 0 && totalTagihan >= 0) {
            ...
            // Format detail kalkulasi dengan biaya admin
            document.getElementById('detailKalkulasi').innerHTML = 
                pemakaian.toFixed(2) + ' m³ × Rp ' + tarifPerM3.toLocaleString('id-ID') + 
                ' = Rp ' + biayaPemakaian.toLocaleString('id-ID') +
                '<br>+ Biaya Admin: Rp 5.000' +
                '<br>Total: Rp ' + totalTagihan.toLocaleString('id-ID');
        } else {
    ...
    document.getElementById('tanggal_jatuh_tempo').addEventListener('change', function() {
        ...
        if (dueDate < today && statusSelect.value === \App\Models\Tagihan::STATUS_BELUM_BAYAR) {
